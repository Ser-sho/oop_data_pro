...
def design_report(
    ...
    if analysis_mode == 'Extreme Analysis':
        ...
        systemic = ex.get('systemic_investigation', {}) or {}
        if systemic.get('gate') == 'PASS':
            slides.append({'type':'extreme_systemic_investigation','title':'Extreme Analysis — Systemic Issue Investigation','subtitle':'Evidence-gated Fishbone investigation for a recurring pattern','gate':systemic.get('gate'),'gate_reason':systemic.get('gate_reason'),'candidates':systemic.get('candidates',pd.DataFrame()).head(6).to_dict('records') if isinstance(systemic.get('candidates'),pd.DataFrame) else [],'fishbone':systemic.get('fishbone',pd.DataFrame()).to_dict('records') if isinstance(systemic.get('fishbone'),pd.DataFrame) else [],'likely_causes':systemic.get('likely_causes',pd.DataFrame()).to_dict('records') if isinstance(systemic.get('likely_causes'),pd.DataFrame) else []})
