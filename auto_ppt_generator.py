...
def generate_auto_powerpoint(out_path: Path, blueprint: dict[str,Any]):
    ...
    for s in blueprint.get('slides',[]):
        ...
        elif typ=='extreme_systemic_investigation':
            candidates=s.get('candidates',[]) or []; fish=s.get('fishbone',[]) or []; causes=s.get('likely_causes',[]) or []
            box(slide,0.65,1.42,11.7,0.78,LIGHT,LIGHT)
            text(slide,0.92,1.64,2.1,0.3,'Systemicity gate',9,True,DARK)
            text(slide,3.0,1.57,1.0,0.4,str(s.get('gate','HOLD')),16,True,GREEN)
            text(slide,4.0,1.58,8.0,0.42,str(s.get('gate_reason','')),8.5,False,DARK)
            box(slide,0.65,2.38,5.65,3.95,WHITE,MID); text(slide,0.92,2.65,5.0,0.3,'Recurring pattern detected',12,True,NAVY)
            y=3.08
            for r in candidates[:5]:
                text(slide,0.92,y,2.6,0.34,str(r.get('issue_pattern',''))[:32],8.5,True,DARK)
                text(slide,3.55,y,1.0,0.34,f"{r.get('cases',0)} cases",8,False,DARK)
                text(slide,4.55,y,1.45,0.34,f"{r.get('active_months',0)} months",8,False,DARK); y+=0.58
            box(slide,6.55,2.38,5.8,3.95,WHITE,MID); text(slide,6.82,2.65,5.1,0.3,'Fishbone evidence map',12,True,NAVY)
            y=3.08
            for r in fish[:6]:
                text(slide,6.82,y,2.25,0.30,str(r.get('fishbone_category',''))[:27],7.8,True,DARK)
                text(slide,9.08,y,2.95,0.48,str(r.get('evidence_signal',''))[:95],7.8,False,DARK); y+=0.52
            add_insight_box(slide,0.65,6.48,11.7,0.45,'Systemicity passed, but candidate causes remain hypotheses. Confirm root cause only after the Five Whys and operational/process evidence are validated.')
