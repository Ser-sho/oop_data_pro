...
def build_intelligence(analysis: dict[str, Any], total_wards: int | None = None) -> dict[str, Any]:
    ...
    # Keep the evidence register traceable even when the selected reporting date
    # contains no usable channel rows. E-004 remains a valid evidence definition,
    # but its detail must explicitly state that the channel metric is unavailable.
    if not channel.empty:
        pass
    else:
        d("E-004","F-004","Channel activity", "Not available", "No valid Channel records were available for the selected reporting date")
