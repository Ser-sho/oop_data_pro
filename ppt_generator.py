...
def generate_powerpoint(template_path, output_path, analysis, intelligence, municipality, reporting_date, period_covered, close_time, audience_view=None, voc_analysis=None):
    ...
    s=prs.slides[5]
    if voc_analysis:
        vals=[voc_analysis.get('responses',0),voc_analysis.get('happiness','Not calculated'),voc_analysis.get('resolved','Not calculated'),voc_analysis.get('survey_ready','Not calculated'),'Available']
    else: vals=[0,'Not calculated',0,'Not calculated','Not supplied']
    _set_by_index(s,[10,13,16,19,22],vals)
    if not voc_analysis:
        _set_text(s.shapes[50],'No VoC master source supplied. Happiness, effort, satisfaction and promotion are not inferred.')
    else:
        svc=voc_analysis.get('top_service') or 'No dominant service'
        hdet=voc_analysis.get('happiness_detail_pct')
        extra=f' Supporting happiness detail: {hdet:.1f}% positive.' if isinstance(hdet,(int,float)) else ''
        _set_text(s.shapes[50],f'VoC master survey: {voc_analysis.get("responses",0):,} responses; leading service: {svc}.{extra} Detailed effort, satisfaction, promotion and reasons are carried in the addendum.')
