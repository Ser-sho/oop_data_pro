...
from analytical_intelligence import build_analytical_intelligence
...
with st.sidebar:
    ...
    profile_name = st.selectbox("Entity profile", list(ENTITY_PRESETS.keys()), index=0, help="Presentation/planning vocabulary only. The uploaded data remains authoritative for metrics.")
...
entity_context = build_entity_context(department, requesting_team, profile_name)
entity_qa = pd.DataFrame(validate_entity_context(entity_context))
...
st.subheader("Entity & team configuration")
st.write({"department": department, "entity_profile": profile_name, "requesting_team": requesting_team, "report_identity": entity_context["report_identity"]})
with st.expander("Configuration QA"):
    st.dataframe(entity_qa, use_container_width=True, hide_index=True)

st.subheader("Dynamic report planner")
st.write({"department": department, "requesting_team": requesting_team, "entity_profile": profile_name, "reporting_period": reporting_period.label, "audience": audience})
...
if ai_qa_pass:
    ...
    st.error("🔴 Analytical intelligence QA requires review.")
st.dataframe(analytical_intelligence["findings"], use_container_width=True, hide_index=True)
with st.expander("V3.4 recommendations and evidence"):
    st.dataframe(analytical_intelligence["recommendations"], use_container_width=True, hide_index=True)
...
if di_qa_pass:
    ...
    st.error("🔴 Decision/action governance QA requires review.")
st.dataframe(decision_intelligence["actions"], use_container_width=True, hide_index=True)
with st.expander("Escalation register and decision controls"):
...
blueprint = design_report(
    ...
    analytical_intelligence=analytical_intelligence, decision_intelligence=decision_intelligence
)
...
if st.button("Generate Excel Analytical Addendum"):
    ...
    try:
        ...
        intelligence_with_qa["v34_findings"] = analytical_intelligence.get("findings")
        intelligence_with_qa["v34_recommendations"] = analytical_intelligence.get("recommendations")
        intelligence_with_qa["v34_evidence"] = analytical_intelligence.get("evidence")
        intelligence_with_qa["v34_qa"] = analytical_intelligence.get("qa")
