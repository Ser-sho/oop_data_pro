...
def generate_addendum(output_path: str|Path, source_filename: str, sheet_name: str, analysis: dict[str,Any], intelligence: dict[str,Any], municipality: str, reporting_date, period_covered: str, close_time: str, voc_analysis=None, entity_context: dict[str,Any] | None = None) -> Path:
    output_path=Path(output_path); output_path.parent.mkdir(parents=True,exist_ok=True)
    s=analysis["summary"]
    context=pd.DataFrame([
        ["Report", f"{(entity_context or {}).get("entity", {}).get("department", "OOP Corridor")} Operations & Analytics Report"],["Department / entity", (entity_context or {}).get("entity", {}).get("department", "Not specified")],["Requesting team", (entity_context or {}).get("team", {}).get("team", "Not specified")],["Entity profile", (entity_context or {}).get("entity", {}).get("profile_name", "Generic")],["Municipality / corridor",municipality],["Reporting date",reporting_date.isoformat()],
    ...
    with pd.ExcelWriter(output_path,engine="openpyxl") as writer:
        ...
        _write_df(writer,intelligence.get("findings"),"Findings"); _write_df(writer,intelligence.get("recommendations"),"Recommendations"); _write_df(writer,intelligence.get("evidence"),"Evidence Register"); _write_df(writer,intelligence.get("evidence_details"),"Evidence Detail"); _write_df(writer,intelligence.get("qa"),"Traceability QA"); _write_df(writer,intelligence.get("v34_findings"),"V3.4 Findings"); _write_df(writer,intelligence.get("v34_recommendations"),"V3.4 Recommendations"); _write_df(writer,intelligence.get("v34_evidence"),"V3.4 Evidence"); _write_df(writer,intelligence.get("v34_qa"),"V3.4 QA"); _write_df(writer,intelligence.get("v36_actions"),"V3.6 Action Register"); _write_df(writer,intelligence.get("v36_escalations"),"V3.6 Escalation Register"); _write_df(writer,intelligence.get("v36_qa"),"V3.6 Decision QA"); _write_df(writer,intelligence.get("final_report_qa"),"Final Report QA")
