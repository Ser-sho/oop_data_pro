...
def generate_addendum(
    ...
    if voc_analysis:
        ...
        _write_table(ws, 1, summary, 'VoC summary')
        if isinstance(voc_analysis.get('details'), pd.DataFrame) and detail != 'Executive':
            _write_table(ws, ws.max_row + 1, voc_analysis['details'].head(top_n), 'VoC detail / follow-up queue')
        _style_sheet(ws, 'Green')
    else:
        _write_table(ws, 1, pd.DataFrame({'Note':['No Voice of Citizen source was supplied for this report. Metrics are not inferred.']}), 'VoC availability')
